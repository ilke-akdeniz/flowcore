...
func rowToStep(row pgx.CollectableRow) (StepDefinition, error) {
	...
	err := row.Scan(&s.ID, &s.WorkflowDefinitionID, &s.WorkflowStatusDefinitionID, &s.AssigneeID, &s.Name)
	return s, err
}

func insertStep(ctx context.Context, q querier, s StepDefinition) error {
	_, err := q.Exec(ctx,
		`insert into flowcore.step_definition
		 (id, workflow_definition_id, workflow_status_definition_id, assignee_id, name)
		 values ($1, $2, $3, $4, $5)`,
		s.ID, s.WorkflowDefinitionID, s.WorkflowStatusDefinitionID, s.AssigneeID, s.Name)
...
func getStepRow(ctx context.Context, q querier, id uuid.UUID) (StepDefinition, error) {
	...
	err := q.QueryRow(ctx,
		...
		id).Scan(&s.ID, &s.WorkflowDefinitionID, &s.WorkflowStatusDefinitionID, &s.AssigneeID, &s.Name)
